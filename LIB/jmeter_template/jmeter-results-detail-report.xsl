		...
		<table class="details" border="0" cellpadding="5" cellspacing="2"
			...
			<tr valign="top">
				...
				<xsl:variable name="allFailureCount"
					select="count(/testResults/httpSample[attribute::s='false'])" />
				<xsl:variable name="allSuccessCount"
					select="count(/testResults/httpSample[attribute::s='true'])" />
		...
		<table class="details" border="0" cellpadding="5" cellspacing="2"
			...
			<xsl:for-each select="/testResults/httpSample[not(@lb = preceding::*/@lb)]">
				...
				<xsl:variable name="count"
					select="count(../httpSample[@lb = current()/@lb])" />
				<xsl:variable name="failureCount"
					select="count(../httpSample[@lb = current()/@lb][attribute::s='false'])" />
				<xsl:variable name="successCount"
					select="count(../httpSample[@lb = current()/@lb][attribute::s='true'])" />
				<xsl:variable name="successPercent" select="$successCount div $count" />
				<xsl:variable name="totalTime"
					select="sum(../httpSample[@lb = current()/@lb]/@t)" />
				<xsl:variable name="averageTime" select="$totalTime div $count" />
				<xsl:variable name="minTime">
					<xsl:call-template name="min">
						<xsl:with-param name="nodes"
							select="../httpSample[@lb = current()/@lb]/@t" />
					</xsl:call-template>
				</xsl:variable>
			...
			<xsl:for-each select="/testResults/httpSample[not(@lb = preceding::*/@lb)]">
				...
				<xsl:if test="$failureCount > 0">
					...
					<table class="details" border="0" cellpadding="5"
						...
						<xsl:for-each
							select="/testResults/httpSample[@lb = current()/@lb][attribute::s='false']">
							<tr>
								<td>
